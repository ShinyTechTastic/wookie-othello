...
abstract class GamePlayer {
  def choose( list:Set[Move] , b:Board ):(Move)
}

object RandomPlayer extends GamePlayer {
  override def choose( list:Set[Move] , b:Board ):(Move) = Random.shuffle(list.toList).head
}

abstract class ScoringPlayer extends GamePlayer {
  def score( move:(Move) , b:Board ):Double
  
  def findBestMove( moves:Set[Move] , b:Board ):(Double,(Move)) = {
  ...
  override def choose( list:Set[Move] , b:Board):(Move) = findBestMove( list , b )._2
}
